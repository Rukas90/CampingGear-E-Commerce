...
public class RefreshService(
    IOptions<RefreshOptions> options, IRefreshRepository refreshRepository, IPasswordHasher passwordHasher) : IRefreshService
{
    private readonly RefreshOptions _options = options.Value;
    
    public async Task<string> GenerateToken(Id<Customer> customerId, Id<RefreshTokenFamily>? familyId = null, CancellationToken ct = default)
    {
    ...
    public async Task<Result<RefreshToken>> FindRefreshToken(string token, CancellationToken ct)
    {
        ...
        {
            return AuthProblems.RefreshTokenNotFound;
        }
