...
public static class OptionsRegistrationExtensions
{
    extension(IServiceCollection services)
    {
        public IServiceCollection ConfigureAppOptions<T>(IConfiguration configuration) 
            => services.ConfigureAppOptionsFromAssemblies(configuration, typeof(T).Assembly);

        public IServiceCollection ConfigureAppOptionsFromAssemblies(IConfiguration configuration, params Assembly[] assemblies)
        {
            ...
            if (method is null)
            {
                throw new Exception("Configure method not found");
            }
